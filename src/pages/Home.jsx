    ...
    <div className="flex-1">
      <section className="max-w-5xl mx-auto px-4 py-16 flex flex-col items-center text-center gap-8">
        <div className="flex flex-col items-center gap-8 w-full">
          ...
          {event?.description && (
            <p className="max-w-xl text-neutral-400">{event.description}</p>
          )}
          {event?.ctaText && event?.ctaUrl && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-block bg-accent text-white font-semibold px-6 py-3 rounded uppercase tracking-wide text-sm hover:brightness-110 transition"
            >
              {event.ctaText}
            </a>
          )}
